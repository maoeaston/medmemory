...
## 移动端 APP 化重构（2026-06-26 建立）

- 新增 `src/components/layout/` 层：`AppShell.vue`（双布局移动/桌面）+ `BottomTabBar.vue`（5 tab + FAB，固定底栏）+ `MoreDrawer.vue`（底部上滑抽屉）+ `PageContainer.vue`（替代 12 个 view 的 `<main>` wrapper，统一 padding/max-width）
- 断点：≤1023px 走 tab bar，≥1024px 走桌面横向 nav（9 链接）。`App.vue` 简化为 `<AppShell />`，旧 `.app-header/.nav` CSS 全部下沉到 AppShell
- Modal 三变体：`variant?: centered|bottom-sheet|fullscreen`，自动经 `matchMedia('(max-width: 1023px)')` 决议。isVisible 双相关闭（mount 后 nextTick 设 true 触发 enter，handleClose 设 false + 250ms 后 emit close 让 leave 跑完），与 caller `v-if + @close` 模式兼容
- 路由切换方向：`useRouteDirection.ts` 比较 `meta.depth`（0=主 tab，1=二级页，2=event detail），forward→slide-forward / back→slide-back / 同 depth→fade。桌面 ≥1024px 经 CSS `transform:none` 退化为纯 fade。Transition 期间 view `position:absolute`，`.app-main` 需 `position: relative + min-height`
- PWA：`public/manifest.json` + `public/icons/icon-{192,512}.png`（"医"字蓝底，maskable）+ `index.html` 加 manifest link + apple-touch-icon。**不引入 vite-plugin-pwa**（数据在 OPFS，COEP-safe SW 复杂度高，留待后续）
- 新 token：`--safe-area-{top,bottom,left,right}` / `--touch-target-min: 44px` / `--tab-bar-height: 56px` / `--fab-{diameter,raise}` / `--duration-{fast,normal,slow}` / `--ease-{standard,decel,accel}` / `--space-page-padding-mobile: 1rem`
